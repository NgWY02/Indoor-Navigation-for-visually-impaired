...
import 'dart:io';
import 'dart:math';
import 'dart:ui' as ui; // For CompassPainter potentially
...
List reshapeList(List inputList, List<int> shape) {
  ...
  if (total != inputList.length) {
    throw ArgumentError('Cannot reshape a list of length ${inputList.length} to shape $shape');
  }
...
// Copied CompassPainter class from navigation_screen.dart
class CompassPainter extends CustomPainter {
  final double currentHeading;
  final Map<int, bool> scannedDirections;
  final List<double> targetAngles;
  final double targetAngle; // This is the specific angle the painter should highlight/focus on
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var angle in targetAngles) {
      ...
      if (scannedDirections[angle.toInt()] == true) {
        ...
        dotRadius = 5.0;
      } else if ((angle - targetAngle).abs() < 0.1) { // Highlight current target
        dotColor = Colors.blueAccent; // Use a distinct color for the painter's target
        dotRadius = 7.0;
      } else {
    ...
    final bool isPainterTargetAlreadyScanned = scannedDirections[targetAngle.toInt()] ?? false;
    if (!isPainterTargetAlreadyScanned) {
      final sectorPaint = Paint()
        ..color = Colors.blueAccent.withOpacity(0.3)
        ..style = PaintingStyle.fill;
      final targetRadianForSector = (targetAngle - 90) * pi / 180;
      final startAngleForSector = targetRadianForSector - (threshold * pi / 180);
      final sweepAngleForSector = (threshold * 2 * pi / 180);
      final rect = Rect.fromCircle(center: center, radius: radius);
      canvas.drawArc(rect, startAngleForSector, sweepAngleForSector, true, sectorPaint);
    }
  }
  ...
  bool shouldRepaint(CompassPainter oldDelegate) {
    return oldDelegate.currentHeading != currentHeading ||
           ...
           !mapEquals(oldDelegate.scannedDirections, scannedDirections); // Use mapEquals for Map comparison
  }
}
...
class NavigationLocalizationScreen extends StatefulWidget {
  ...
  const NavigationLocalizationScreen({Key? key, required this.camera}) : super(key: key);

  @override
  _NavigationLocalizationScreenState createState() => _NavigationLocalizationScreenState();
}

class _NavigationLocalizationScreenState extends State<NavigationLocalizationScreen> {
  ...
  List<Map<String, dynamic>> _scan360Results = []; // To store {'embedding': List<double>, 'heading': double}
  double? _currentHeading;
  StreamSubscription<CompassEvent>? _compassSubscription360;
  Timer? _scan360Timer;
  final int _totalScansNeeded = 18; // 360 / 20 = 18 intervals
  final List<double> _targetAngles = List.generate(18, (i) => i * 20.0); // 0, 20, 40, ..., 340
  ...
  int? _angleBeingDwelledOn; // The specific target angle (e.g., 0, 20, 40) currently being aimed at
  DateTime? _dwellStartTime;
  final Duration _requiredDwellDuration = const Duration(milliseconds: 750); // e.g., 0.75 seconds
      ...
      if (mounted) {
        setState(() {
          ...
          if (_storedEmbeddings.isEmpty) {
            ...
            _speak("No locations found in the database. Please add some first.");
          } else {
  ...
  Future<void> _speak(String text) async {
    if (_isSpeaking || text.isEmpty || text == _lastSpokenText && text != "Scan complete. Processing results.") {
  ...
  void _reset360ScanState() {
    _scannedDirections = Map.fromIterable(
      _targetAngles,
      key: (angle) => angle.toInt(),
      value: (_) => false
    );
    _scanCount360 = 0;
    _scan360Results = [];
    _scanGuidance = "Tap 'Start 360 Scan'";
    _recognizedPlace = _storedEmbeddings.isEmpty ? "No locations found." : "Ready to scan.";
    _confidence = 0.0;
  }
  ...
  void _start360Scan() {
    ...
    _scan360Timer = Timer.periodic(const Duration(milliseconds: 750), (timer) { // Check more frequently
      if (!_isScanning360) {
        timer.cancel();
        return;
      }
  ...
  void _updateScanGuidanceText() {
    String newGuidance = ""; // Initialize to prevent uninitialized access if conditions not met
    ...
    if (!_isScanning360 || _currentHeading == null) {
      ...
      newIcon = _isScanning360 ? Icons.hourglass_empty : Icons.play_circle_outline;
       // Don't speak "Waiting for compass" too often, only if state changes to this.
      if (newGuidance != _scanGuidance) {
          shouldSpeakGuidance = _isScanning360; // Speak if actively scanning and waiting
      }
      setState(() {
        _scanGuidance = newGuidance;
        // _guidanceIcon = newIcon; // If we add an icon state variable
        if (_isScanning360) {
             _currentPainterTargetAngle = _targetAngles.isNotEmpty ? _targetAngles.first : 0.0;
        }
      });
    ...
    String actualTargetNameForText = _getDirectionName(actualNextTargetForTextGuidance.toInt());
    double diffToActualNextTarget = _getAngleDifference(_currentHeading!, actualNextTargetForTextGuidance);
    ...
    if (diffToActualNextTarget <= _captureAngleThreshold) {
      ...
      String turnDir = _getTurnDirection(_currentHeading!, actualNextTargetForTextGuidance);
      newGuidance = "Turn $turnDir to $actualTargetNameForText (${diffToActualNextTarget.round()}°)";
    ...
    bool painterTargetChanged = _currentPainterTargetAngle != actualNextTargetForTextGuidance;

    if (newGuidance != _scanGuidance || painterTargetChanged) {
      setState(() {
           _scanGuidance = newGuidance;
           _currentPainterTargetAngle = actualNextTargetForTextGuidance; // Ensure painter target is consistent
           // _guidanceIcon = newIcon; 
        });
  ...
  Future<void> _checkAndCaptureFor360Scan() async {
    if (!_isScanning360 || _currentHeading == null || !_cameraController.value.isInitialized) {
      // If not scanning or no heading, reset any active dwelling state
      if (_angleBeingDwelledOn != null) {
        _debugLog("Scan conditions not met, resetting dwell on $_angleBeingDwelledOn");
    ...
    if (currentlyAlignedUnscannedAngle != null) {
      ...
      if (_angleBeingDwelledOn != currentlyAlignedUnscannedAngle) {
        ...
        _debugLog("Now dwelling on target: $currentlyAlignedUnscannedAngle. Current heading: $_currentHeading");
        ...
        if (_dwellStartTime != null && DateTime.now().difference(_dwellStartTime!) >= _requiredDwellDuration) {
          _debugLog("Dwell time met for $currentlyAlignedUnscannedAngle. Attempting capture.");
          ...
          if (success) {
            if (mounted) {
              ...
              _debugLog('Successfully scanned angle: $currentlyAlignedUnscannedAngle. Scan count: $_scanCount360/$_totalScansNeeded. Scanned directions map: $_scannedDirections');

              String successMessage = "${_getDirectionName(currentlyAlignedUnscannedAngle)} scanned (${_scanCount360}/${_totalScansNeeded})";
              // _debugLog(successMessage); // Original log, now covered by the one above mostly

              int newNextTargetAngle = _targetAngles.firstWhere(
                  (a) => _scannedDirections[a.toInt()] == false, 
                  orElse: () => -1
              ).toInt();
              ...
              _speak("${_getDirectionName(currentlyAlignedUnscannedAngle)} scanned.");
              HapticFeedback.mediumImpact();
            }
          } else {
            _debugLog("Capture failed for $currentlyAlignedUnscannedAngle despite dwell.");
          }
          // Reset dwell state AFTER capture attempt (success or fail) for this angle
          _angleBeingDwelledOn = null;
          _dwellStartTime = null;
        } else {
          _debugLog("Still dwelling on $currentlyAlignedUnscannedAngle, time not met. Started: $_dwellStartTime, Now: ${DateTime.now()}");
      ...
      if (_angleBeingDwelledOn != null) {
        _debugLog("Moved out of dwell zone for $_angleBeingDwelledOn. Resetting dwell state.");
        _angleBeingDwelledOn = null; // Reset if user moved away from a dwelling target
        _dwellStartTime = null;
      }
    }
  ...
  Future<bool> _captureAndStoreEmbedding() async {
    ...
    try {
      ...
      var outputBuffer = List<double>.filled(1 * 1280, 0.0); // Matches model output
  ...
  List<List<List<List<double>>>> _prepareImageInput(img.Image image) {
    ...
    final input = List.generate(1, (_) => List.generate(height, (_) => 
  ...
  void _process360ScanResults() {
    _debugLog("Processing ${_scan360Results.length} scan results for majority vote.");
    if (mounted) {
        setState(() {
    ...
    Map<String, List<double>> similaritiesPerLocation = {}; // To average confidence for majority
    ...
    for (var scanData in _scan360Results) {
      ...
      _storedEmbeddings.forEach((locationName, storedEmbedding) {
        ...
        double similarity = _calculateCosineSimilarity(scannedEmbedding, storedEmbedding);
      ...
      const double voteThreshold = 0.3; // Can be adjusted, similar to previous temporary threshold
      if (bestMatchForThisScan != "Unknown" && highestSimilarityForThisScan > voteThreshold) {
        voteCounts[bestMatchForThisScan] = (voteCounts[bestMatchForThisScan] ?? 0) + 1;
        
        // Store similarity for averaging later
        similaritiesPerLocation.putIfAbsent(bestMatchForThisScan, () => []).add(highestSimilarityForThisScan);
        _debugLog("Vote for $bestMatchForThisScan with similarity ${(highestSimilarityForThisScan * 100).round()}%");
      } else {
         _debugLog("Scan point did not meet threshold or no match. Similarity: ${(highestSimilarityForThisScan * 100).round()}% for $bestMatchForThisScan");
      }
    }

    if (voteCounts.isEmpty) {
      _debugLog("No location received enough votes above threshold.");
      if (mounted) setState(() => _recognizedPlace = "Unknown Location (No consensus)");
    ...
    String majorityLocation = voteCounts.entries.reduce((a, b) => a.value > b.value ? a : b).key;
    int majorityVotes = voteCounts[majorityLocation]!;
    
    double averageConfidenceForMajority = 0.0;
    if (similaritiesPerLocation.containsKey(majorityLocation) && similaritiesPerLocation[majorityLocation]!.isNotEmpty) {
        List<double> scores = similaritiesPerLocation[majorityLocation]!;
        averageConfidenceForMajority = scores.reduce((a,b) => a+b) / scores.length;
    }

    _debugLog("Majority location: $majorityLocation with $majorityVotes votes. Avg Confidence: ${ (averageConfidenceForMajority * 100).round()}%");
    
    if (mounted) {
      setState(() {
        _recognizedPlace = majorityLocation;
        _confidence = averageConfidenceForMajority; // Use average similarity of winning votes
      });
    }
    _speak("Based on the 360 scan, you are likely at $majorityLocation with ${(_confidence * 100).round()}% confidence.");
    // Reset scan state for next scan, but keep result displayed
     if (mounted) {
      setState(() {
        // Only reset scan-specific tracking, not the result itself
        _scannedDirections = Map.fromIterable(_targetAngles, key: (angle) => angle.toInt(), value: (_) => false);
        _scanCount360 = 0;
        _scan360Results = [];
  ...
  Widget build(BuildContext context) {
    // Loading screen similar to navigation_screen.dart
    if (_isLoading) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Location Recognition (360°)'), // Keep title relevant
          backgroundColor: Colors.teal, // Keep color consistent
        ),
        body: Center(
          child: Column(
    ...
    if (!_cameraController.value.isInitialized) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Location Recognition (360°)'),
    ...
    // Main UI structure similar to navigation_screen.dart
    return Scaffold(
      appBar: AppBar(
        title: const Text('Location Recognition (360°)'),
        backgroundColor: Colors.teal,
      ...
      body: Column(
        children: [
          Expanded(
            flex: 3, // Matches navigation_screen.dart
            child: Stack(
              children: [
                ...
                if (_isScanning360)
                  Container(
                    ...
                    child: Center(child: _buildScanningOverlayWidget()), // Use new helper
                  ),
          ...
          // Location information panel (similar to navigation_screen.dart)
          Container(
            ...
            color: Colors.teal.shade100, // Match AppBar color scheme
              ...
              child: Column(
                children: [
                  ...
                  _isScanning360 ? (_scanGuidance.isNotEmpty ? _scanGuidance : "Scanning...") : _recognizedPlace,
          ...
          // Controls (similar to navigation_screen.dart)
          Padding(
            ...
            child: Row(
              ...
              children: [
                ...
                ElevatedButton.icon(
                  icon: Icon(_isScanning360 ? Icons.stop_circle_outlined : Icons.threesixty),
                  label: Text(_isScanning360 ? "Stop Scan" : "Start 360° Scan"),
                  onPressed: _isLoading ? null : (_isScanning360 ? _stop360Scan : _start360Scan),
                  style: ElevatedButton.styleFrom(
                    backgroundColor: _isScanning360 ? Colors.redAccent : Colors.teal,
                    foregroundColor: Colors.white,
                    padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 12),
                    textStyle: const TextStyle(fontSize: 16)
                        ),
  ...
  // New helper widget for the scanning overlay, similar to _buildDirectionGuidance in navigation_screen.dart
  Widget _buildScanningOverlayWidget() {
    if (_currentHeading == null && _isScanning360) {
      return const Text("Waiting for compass...", style: TextStyle(color: Colors.white, fontSize: 18));
    }
    ...
    if (_currentHeading != null) {
      angleForPainterHighlight = (_currentHeading! / 15.0).round() * 15.0;
      ...
      angleForPainterHighlight = _targetAngles.isNotEmpty ? _targetAngles.first : 0.0; // Default
    }
    ...
    IconData guidanceIcon = Icons.explore; // Default icon before specific guidance
    String guidanceText = "Align with target"; // Default text

    if (_currentHeading != null) { 
      if (diffToActualNextTarget <= _captureAngleThreshold) {
        guidanceIcon = Icons.check_circle;
        guidanceText = "Perfect! Hold steady at $actualTargetNameForText"; // Text for visual display
      } else {
        String turnDir = _getTurnDirection(_currentHeading!, actualNextTargetForTextGuidance);
        guidanceIcon = turnDir == "right" ? Icons.arrow_forward : Icons.arrow_back;
        guidanceText = "Turn $turnDir to $actualTargetNameForText (${diffToActualNextTarget.round()}°)"; // Text for visual display
      }
    } else if (_isScanning360) {
      guidanceIcon = Icons.hourglass_empty;
      guidanceText = "Waiting for compass to guide you to $actualTargetNameForText";
    }
    ...
    return Column(
      ...
      children: [
        ...
        SizedBox(
          ...
          child: CustomPaint(
            painter: CompassPainter(
              ...
              targetAngle: angleForPainterHighlight, // Painter's highlight follows current heading segment
        ...
        Text("$completedDirections/$_totalScansNeeded", style: const TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold)),
        const SizedBox(height: 8),
        Icon(guidanceIcon, color: Colors.white, size: 40),
        Text(guidanceText, style: const TextStyle(color: Colors.white, fontSize: 16), textAlign: TextAlign.center),
        const SizedBox(height: 20),
        // Detailed text guidance below compass
        Text("$remainingDirections directions remaining", style: const TextStyle(color: Colors.white, fontSize: 14)),
        const SizedBox(height: 10),
         Padding(
           padding: const EdgeInsets.symmetric(horizontal: 40.0), 
           child: LinearProgressIndicator(
            value: _totalScansNeeded > 0 ? completedDirections / _totalScansNeeded : 0,
