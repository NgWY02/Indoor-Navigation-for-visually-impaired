...
class _MapManagementState extends State<MapManagement> {
  ...
  void _recordPath() {
    ...
    _showNodeSelectionDialog();
  }

  Future<void> _showNodeSelectionDialog() async {
    try {
      // Get map details with nodes
      final mapDetails = await _supabaseService.getMapDetails(_selectedMap!['id']);
      final List<dynamic> nodes = mapDetails['map_nodes'] ?? [];

      if (nodes.length < 2) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('This map needs at least 2 nodes to record a path'),
      ...
      // Show dialog to select start and end nodes
      showDialog(
        context: context,
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Error loading map nodes: $e'),
...
// Path Selection Dialog for choosing start and end nodes
class _PathSelectionDialog extends StatefulWidget {
  final String mapId;
  final List<dynamic> nodes;
  final Function(Map<String, dynamic>, Map<String, dynamic>) onPathSelected;

  const _PathSelectionDialog({
    Key? key,
    required this.mapId,
    required this.nodes,
  ...
  @override
  _PathSelectionDialogState createState() => _PathSelectionDialogState();
}

class _PathSelectionDialogState extends State<_PathSelectionDialog> {
  Map<String, dynamic>? _selectedStartNode;
  Map<String, dynamic>? _selectedEndNode;

  @override
  Widget build(BuildContext context) {
    return AlertDialog(
      title: Text('Select Path Route'),
      content: Container(
        ...
        child: Column(
          ...
          children: [
            Text(
              'Choose the start and end points for your path recording:',
              style: TextStyle(fontSize: 14, color: Colors.grey[600]),
            ),
            SizedBox(height: 20),
            
            // Start Node Selection
            Text('From (Start):', style: TextStyle(fontWeight: FontWeight.bold)),
            SizedBox(height: 8),
            Container(
              decoration: BoxDecoration(
                ...
                child: DropdownButton<Map<String, dynamic>>(
                  ...
                  items: widget.nodes.map((node) {
                    return DropdownMenuItem<Map<String, dynamic>>(
                      ...
                      child: Padding(
                        padding: EdgeInsets.symmetric(horizontal: 12),
                        child: Text(node['name'] ?? 'Unnamed Node'),
                      ),
                    );
                  }).toList(),
                  onChanged: (node) {
                    setState(() {
                      ...
                      // Clear end node if it's the same as start
                      if (_selectedEndNode != null && 
                          _selectedEndNode!['id'] == node!['id']) {
            ...
            // End Node Selection
            Text('To (End):', style: TextStyle(fontWeight: FontWeight.bold)),
            SizedBox(height: 8),
            Container(
              decoration: BoxDecoration(
                border: Border.all(color: Colors.grey[300]!),
              ...
              child: DropdownButtonHideUnderline(
                child: DropdownButton<Map<String, dynamic>>(
                  ...
                  items: widget.nodes
                      .where((node) => _selectedStartNode == null || 
                                      node['id'] != _selectedStartNode!['id'])
                      .map((node) {
                    return DropdownMenuItem<Map<String, dynamic>>(
                      value: node,
                      child: Padding(
        ...
        ElevatedButton.icon(
          onPressed: _selectedStartNode != null && _selectedEndNode != null
              ? () {
                  ...
                  widget.onPathSelected(_selectedStartNode!, _selectedEndNode!);
                }
              : null,
